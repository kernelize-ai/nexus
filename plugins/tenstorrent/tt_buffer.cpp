...
TTBuffer::Buffer_sp TTBuffer::makeDeviceBuffer() {
  if (!(getSettings() & NXS_BufferSettings_OnDevice)) {
    ...
    NXSAPI_LOG(nexus::NXS_LOG_NOTE, "TTBuffer: makeDeviceBuffer: tile_size=", tile_size, " size=", size(), " address=", address);
  }
  return buffer;
}
...
nxs_status TTBuffer::copyToHost(void *host_buf) {
  if (address != -1) {
    ...
    NXSAPI_LOG(nexus::NXS_LOG_NOTE, "TTBuffer: copyToHost: tile_size=", tile_size, " size=", size(), " address=", address);
  }
  return NXS_Success;
}